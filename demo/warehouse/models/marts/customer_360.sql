...
with order_stats as (
    select
        customer_id,
        avg(case when order_date >= {{ as_of() }} - 30 then amount_usd end) as avg_order_value_30d,
        count(case when is_completed and order_date >= {{ as_of() }} - 90 then 1 end) as order_count_90d,
...
select
    ...
    case when o.last_order_date < {{ as_of() }} - 90 then true else false end as churned
